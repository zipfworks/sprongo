...
case class Set(fieldVal: Producer[(String, BSONValue)]*) extends UpdateDefinition {
  override def build: BSONDocument = {
    BSONDocument("$set" -> BSONDocument(fieldVal: _*))
  }
}

case class Unset(fields: String*) extends UpdateDefinition {
  override def build: BSONDocument = {
...
case class Inc(fieldVal: Producer[(String, BSONValue)]*) extends UpdateDefinition {
  override def build: BSONDocument = {
